 ...
 * 
 */
@Repository("respuestaCopiaDao")
@Transactional
public class RespuestaCopiaDaoImpl extends Dao<com.ecm.sigap.data.model.RespuestaCopia> {
	...
	protected Criteria createCriteria(List<Criterion> restrictions, List<Order> orders, ProjectionList projections,
		...
		Criteria criteria = getSession().createCriteria(RespuestaCopia.class);

		criteria.createAlias("status", "status");
		criteria.createAlias("respuesta", "la_respuesta");
