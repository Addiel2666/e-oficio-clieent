...
/**
 * Manejador en base de datos de objetos {@link DocumentoAntefirmaRespuesta}.
 * 
 * @author Alfredo Morales
 * @version 1.0
 * 
 */
@Service("documentoAntefirmaRespuestaService")
public class DocumentoAntefirmaRespuestaManagerImpl extends ManagerImpl<DocumentoAntefirmaRespuesta> {
	 ...
	 */
	@Autowired
	@Qualifier("documentoAntefirmaRespuestaDao")
	@Override
	protected void setDao(EntityDAO<DocumentoAntefirmaRespuesta> dao) {
		super.setDao(dao);
	}
	...
	/** Manejador para el tipo {@link com.ecm.sigap.data.model.DocumentosRespuesta} */
	@Autowired
	 ...
	 */
	@Override
	public DocumentoAntefirmaRespuesta fetch(Serializable id) {
		DocumentoAntefirmaRespuesta fetch = super.fetch(id);
		if (null != fetch) {
			completeItem(fetch);
		}
		return fetch;
	 ...
	 * @param fetch
	 */
	private void completeItem(DocumentoAntefirmaRespuesta fetch) {
		...
		if("R".equals(fetch.getDocumentoAntefirmaKey().getTipo())) {
			try {
				DocumentoRespuesta docMarca_tmp = mngrDocsRespuesta.fetch(fetch.getDocumentoAntefirmaKey().getObjectId());
				if(docMarca_tmp != null)
					fetch.setFechaMarca(docMarca_tmp.getFechaMarca());
			} catch (Exception e) {
				// no existe un documento con ese getObjectId
			}
		}
	...
	public List<?> search(List<Criterion> restrictions, List<Order> orders, ProjectionList projections,
		...
		List<DocumentoAntefirmaRespuesta> search = (List<DocumentoAntefirmaRespuesta>) super.search(restrictions,
				orders, projections, fetchSize, firstResult);

		if (search != null && !search.isEmpty() && search.get(0) instanceof DocumentoAntefirmaRespuesta)
			for (DocumentoAntefirmaRespuesta fetch : search)
				completeItem(fetch);
