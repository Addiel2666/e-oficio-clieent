 ...
 * 
 */
public class StringSequenceGeneratorObjetos implements IdentifierGenerator {

	/**
	 * Log de suscesos.
	 */
	private static final Logger log = LogManager.getLogger(StringSequenceGeneratorObjetos.class);
	/** */
	...
	public Serializable generate(SharedSessionContractImplementor session, Object object) throws HibernateException {
		...
		try {
			...
			if ("POSTGRE".equalsIgnoreCase(db_type) || "POSTGRESQL".equalsIgnoreCase(db_type)) {
				...
				query = "select nextval('{SIGAP_SCHEMA}.SECOBJETOS') AS TABLE_PK";

			} else if ("SQLSERVER".equalsIgnoreCase(db_type)) {

				query = "select (NEXT VALUE FOR {SIGAP_SCHEMA}.SECOBJETOS) AS TABLE_PK";

			} else {

				query = "select {SIGAP_SCHEMA}.SECOBJETOS.nextval AS TABLE_PK from dual";
